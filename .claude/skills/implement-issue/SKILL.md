...
- 画面が変わる変更(新機能・UI に触れる fix)では、関係する Maestro フローを流す。無ければ追加する。流したフローと成否は PR 本文の表に書く(adr/0014)。ロジックだけ・docs だけの変更では流さない。
- Issue の「全体リグレッションが要る」にチェックがあれば、`/regression-test` を実行し、結果表を PR 本文に貼る。
