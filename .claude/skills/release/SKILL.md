...
バンプ PR はコードを変えないので、いまの main がそのままストアに出るものになる。
PR を作る前に、それを E2E で確認する(基準の正は `docs/adr/0014`)。

- **major・minor**:`/regression-test` を実行し、出てきた結果表をバンプ PR の本文に貼る。
- **patch**:手順 1 で読んだ変更に関係するフローだけを流し
  (`maestro test .maestro/<機能>/<フロー>.yaml`)、フローと成否を PR 本文の表に書く。
  関係するフローが無ければ、そう書く。
- 失敗があれば PR を作らずに止まり、ユーザーに確認する。
