...
- 関連: adr/0006, adr/0014, adr/0015, adr/0016, adr/0020（本 ADR で置き換え）, Issue #74, Issue #76
...
- **中身のタップは `keyboardShouldPersistTaps="handled"`**：既定（`never`）だと、
  キーボード表示中の最初のタップがキーボードを閉じるだけで消費され、「決定」が押せない。
- **シートを開いている間は、うしろの画面のキーボード回避を止める**：シートの中の
...
- **表示中にアンマウントしない**：ネイティブのシートは、表示中に React 側で
