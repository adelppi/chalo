...
- **ネイティブの部品・ヘッダーを優先**（`Issue #16`）[確定]：自作で「ネイティブ感」が薄れる箇所は OS 標準に寄せる。戻る／編集／削除は画面内の自作ボタンではなく Expo Router のナビゲーションヘッダー（`Stack.Screen` の `header*` option）に統合する。プラン作成・編集（C-3/D-2）はシートやモーダルではなく、戻るで閉じる通常のプッシュ遷移のフル画面にする（デザイン TURN 5）。時刻選択は `@react-native-picker/picker` の iOS ネイティブのホイールピッカーを使う（Claude Design の色・書体に完全一致しない場合があるのは許容）。ナビゲータの option は `className` 不可のため、下記「逃げ道」に従いオブジェクトで書く。
