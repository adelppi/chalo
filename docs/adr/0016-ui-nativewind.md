...
- **ネイティブの部品を優先しつつ、ヘッダーは画面内で描く**（`Issue #16`→`Issue #17` で更新）[確定]：プラン作成・編集（C-3/D-2）はシートやモーダルではなく、戻るで閉じる通常のプッシュ遷移のフル画面にする（デザイン TURN 5）。時刻選択は `@react-native-picker/picker` の iOS ネイティブのホイールピッカーを使う（Claude Design の色・書体に完全一致しない場合があるのは許容）。戻る／編集／削除は、当初ナビゲーションヘッダー（`Stack.Screen` の `header*` option）への統合とした（`Issue #16`）が、iOS 26 の Liquid Glass ではヘッダーボタンがガラスのカプセルに包まれてデザインと乖離する（編集と削除が1つのカプセルに繋がる）ため、`global/components/ui` の `BackHeader` で画面内に透明の円形ボタンとして描く方式に置き換えた。戻るスワイプはネイティブのまま有効。ナビゲータの option は `className` 不可のため、下記「逃げ道」に従いオブジェクトで書く。
