...
| ステータス | 条件 | 説明 |
|---|---|---|
| `いつかいく`（wish） | 日付なし | いつ行くか未定のまま貯めている状態。何件でも貯められる。 |
...
- 作成直後は、日付があれば `予定`、なければ `いつかいく`。
- 作成すると、ペアが成立していれば**パートナーへ作成通知**が飛ぶ（`domain/notifications.md`）。

### 作成・編集の中断（未保存の変更）
- 作成・編集の途中で戻ろうとしたとき、**未保存の変更があれば保存するかどうかを確認する**。黙って破棄しない。
- 選べるのは「保存する（作成なら作成する）」と「保存せず戻る」の2つ。確認を**閉じただけ**（背景タップ）なら、そのまま編集を続ける。
- 変更が無ければ確認せずに戻る。
- タイトルが空のあいだは保存できないため、確認の「保存する」も押せない（必須はタイトルのみ。上の「作成」）。
- 未保存の変更があるあいだは、**画面左端からのスワイプでは戻れない**（確認を経る戻るボタンに出口を一本化する。実現方式は `adr/0024`）。

### 日付の追加・変更・削除
- `いつかいく` に日付を入れると `予定` になる。
- `予定` の日付を消すと `いつかいく` に戻る。
...
- **おしまい日**＝ボタンを押した日を記録する。
- **すでに `おしまい` のプランにはボタンを出さない**。おしまいのプランの詳細は振り返って見るだけの画面とし、下部のアクション（「おしまいにする」・カレンダー連携）を並べない（カレンダー側は `domain/calendar.md`）。編集・削除はこれまでどおりできる。

### 再オープン（おしまいの取り消し）
- 「おしまいを取り消して `いつかいく`/`予定` に戻す」操作は**設けない**（操作を増やさない）。誤って閉じた場合は再作成で対応する。

## 日付・時刻・期限・おしまい日の関係（重要）

混同しやすいので明文化する。

- **日付（date）**：行く予定の日。任意。`いつかいく`↔`予定` の切り替えに使う。アルバム対象日の第一候補。**この日に向けたリマインドはしない**（通知の主役は「期限」）。
